...
- `MerciScreen.js` / `PointsScreen.js` — ✅ backend OK : réduction % (coupon `reduction_pct`, migration 101), trigger `verifier_streak_client` **ne touche plus `wallets`**, `PointsScreen`/`AccueilScreen` unifiés sur `jalons_client`. **Reste : passe visuelle + vérifier que le texte annonce bien « réduction sur ta prochaine course », pas un crédit.** Ex-C.2 #3.
- `PacksAbonnementScreen.js` — ✅ backend OK : commission par pack (12/8/5 %, migration 102) + taux promo perso (migration 104) + `admin_activer_pack_tr()` + cron. **Reste : passe visuelle** (l'écran affiche « bientôt disponible », pas d'achat in-app — à garder tel quel sauf décision d'ouvrir un flux d'achat). Ex-C.2 #1.
