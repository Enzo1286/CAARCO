...
| Écran | Maquette Stitch | Statut | Lot |
|---|---|---|---|
| `ProfilScreen.js` | `mon_profil_caarco_1`, `_2` | **fait (Session 30, 17/07/2026)** | Backend `sexe`/`date_naissance` déjà OK (colonnes en base + `CHAMPS_PROFIL_AUTORISES` incluant les 2 champs, session 10/07). Écran déjà abouti (composants Lot 0, i18n complet, mode sombre). Passe visuelle Session 30 : DoD « zéro hex » — 3 hex en dur retokenisés (`'#0f141173'`→`colors.nuit+'73'` ; bloc WhatsApp `rgba(37,211,102,0.12)`/`#25D166`→const `VERT_WHATSAPP` + `alpha()`). Ex-bloqué C.3.1. |
...
| `MerciScreen.js` | `merci_caarco` | **fait (Session 31)** | Backend déjà en prod (réduction `reduction_pct`, migration 101, plus de wallet). Passe visuelle Session 31 : texte « réduction sur ta prochaine course » vérifié conforme ; écran déjà tokenisé (aucun hex). Ex-bloqué C.2 #3. |
...
| `client/PointsScreen.js` | `mes_points_caarco_1`, `_2` | **fait (Session 31)** | Backend déjà en prod (réduction, unifié sur `jalons_client`). Passe visuelle Session 31 : texte réduction vérifié ; 2 hex `#e8d0a0` → `alpha(colors.nere, 0.35)` ; typo fr `cette semaine`. Ex-bloqué C.2 #3. |
...
| `transporteur/PacksAbonnementScreen.js` | `packs_abonnement_transporteur` | **fait (Session 31)** | Backend déjà en prod (commission par pack 12/8/5 %, migrations 102/104). Passe visuelle Session 31 : écran « bientôt disponible » gardé tel quel ; DoD zéro hex (4 dégradés → paires de tokens, 2 alpha stringifiés + `#ffffffcc`/`#d6e4d8` retokenisés). Ex-bloqué C.2 #1. |
...
## Écrans ex-bloqués — ⚠️ BACKEND DÉJÀ FAIT (10/07/2026), reste la passe visuelle

⚠️ **CORRECTION Session 30 (17/07/2026)** : cette section listait ces écrans « à implémenter (backend + visuel) » — **c'était périmé**. Tout le **backend** a en réalité été fait le 10/07/2026 (migrations `101→106`, voir la section « Session du 10/07/2026 » en bas de ce fichier) et est **en production**. Il ne reste que la **passe visuelle**, traitée un écran à la fois en Session 30+.

- `ProfilScreen.js` — ✅ backend OK (`sexe`/`date_naissance` : colonnes en base + `CHAMPS_PROFIL_AUTORISES`). **Passe visuelle FAITE (Session 30)** : 3 hex retokenisés (voir la ligne du tableau ci-dessus). Ex-C.3.1.
- `MerciScreen.js` / `PointsScreen.js` — ✅ backend OK : réduction % (coupon `reduction_pct`, migration 101), trigger `verifier_streak_client` **ne touche plus `wallets`**, `PointsScreen`/`AccueilScreen` unifiés sur `jalons_client`. **Passe visuelle FAITE (Session 31)** : texte « réduction sur ta prochaine course » VÉRIFIÉ conforme (`streakChargement`, `streakAtteint`, `jalonPret`, `libelleJalon` → `-X% sur votre prochaine course`) — aucun « +100 XAF crédités », l'ex-bug wallet Session 11 est bien résorbé. `MerciScreen` déjà tokenisé (aucun hex). `PointsScreen` : 2 hex `#e8d0a0` retokenisés → `alpha(colors.nere, 0.35)`. Typo fr `ce semaine` → `cette semaine`. Ex-C.2 #3.
- `PacksAbonnementScreen.js` — ✅ backend OK : commission par pack (12/8/5 %, migration 102) + taux promo perso (migration 104) + `admin_activer_pack_tr()` + cron. **Passe visuelle FAITE (Session 31)** : écran « bientôt disponible » GARDÉ tel quel (CTA → alerte `mePrevenir`/`Fonctionnalité disponible très prochainement !`, aucun flux d'achat ouvert). DoD zéro hex : 4 dégradés hex → paires de tokens (`[bambou,foret]`/`[nere,laterite]`/`[foret,nuit]`), `couleurSoft` elite → `foret10`, 2 alpha stringifiés (`couleur+'1a'`, `brume+'80'`) → `alpha()`, `#ffffffcc` → `alpha(colors.blanc, 0.8)`. Ex-C.2 #1.
...
- ✅ **RÉSOLU (10/07/2026, migration 101, confirmé Session 30)** — trigger `trigger_streak_client`/`verifier_streak_client` : **n'écrit plus dans `wallets`**. La récompense streak est désormais un jalon `reduction_pct` (coupon) consommé sur le prix de la course suivante par `verrouiller_prix_course_creation()`. Le mécanisme de coupon existe et est câblé en prod. Ce 🔴 est clos — le mémo Session 29 qui le rouvrait était périmé.
...
- ✅ **RÉSOLU (Session 33, 17/07/2026) — Cedric a tranché : retirer la section.** `ConfigTarifsScreen.js` section "Commission parrainage" éditait `configurations_systeme.commission_parrainage_pct`, lu **uniquement** par la fonction de libération de séquestre (059, l.170-195 — crédite `wallets`/`transactions_wallet`), flux séquestre MORT (jamais atteint dans le modèle TC). Le parrainage sans commission est figé en V1 (CLAUDE.md : « COOPTATION = pas de pénalité pour le parrain »). **Section + handler `sauvegarderCommission` + 2 états + 7 styles retirés du front** (aucune migration ; bonus : élimine 1 hex en dur `colors.nere+'60'`). Babel OK, zéro orphelin, `comBtnSauv`/`comBtnSauvTexte` conservés (partagés avec la section nuit). Ex-Lot 16, CDC D.20.
- ✅ **PÉRIMÉ → DÉJÀ FAIT EN PROD (constaté Session 33).** La note (Lot 16) disait « colonnes absentes, section cosmétique ». **Faux depuis le 10/07 : migration `103_charge_utile_reelle.sql` (déjà en prod, cf. [[etat-base-supabase-prod]] 085→108) fait exactement ce qui était demandé** — `calculer_prix()` (version active = 103, remplace 097) **lit** `poids_max_kg`/`volume_max_m3` depuis `parametres_tarifs` (l.47-50) et les applique en suppléments poids/volume (l.98-99), avec repli sur seuils par défaut si NULL. Les colonnes **existent en prod** (ajoutées à la main — sinon `calculer_prix` planterait à chaque calcul, or l'app est en prod). ⚠️ **Seul résidu réel = trou de reproductibilité** : aucun `ALTER TABLE … ADD COLUMN` n'est capturé dans un fichier de migration (les colonnes n'existent qu'implicitement via le SELECT de 103). Option (non faite, à valider par Cedric) : ajouter un fichier de migration idempotent `ADD COLUMN IF NOT EXISTS` — no-op contre la prod, ferme le trou repro. NE PAS ré-implémenter le pricing. Ex-Lot 16, CDC D.20.
- 🟡 **PARTIELLEMENT DÉJÀ FAIT (constaté Session 33) — reste 1 UPDATE manuel.** (1) Les 2 templates morts `retrait_traite_tr`/`retrait_refuse_tr` sont **déjà supprimés en prod** par migration `106_templates_notif_documents.sql` (l.16 `DELETE … WHERE cle IN (…)`, déjà appliquée). (2) Le template `credit_wallet` est bien réellement envoyé ; son `corps` déjà assaini (migration 083, "wallet"→"compte") mais sa **`description` (colonne `description`) dit encore** « Envoyé quand l'admin crédite le **wallet** d'un utilisateur. » (texte d'origine 072, jamais mis à jour). Cedric a tranché : actualiser (wallet→compte). **SQL manuel préparé, à exécuter dans le SQL Editor (JAMAIS `db push`)** :
  ```sql
  UPDATE notification_templates
  SET description = 'Envoyé quand l''admin crédite le compte d''un utilisateur.'
  WHERE cle = 'credit_wallet';
  ```
  Ex-Lot 18, CDC D.22.7.
